...
**Status:** For review before Stage A item 5 (the auth and audit contexts) is built. Revised 14 September 2026 (Document 27, closing Document 25's findings 4.1, 4.2, and part of the §6/§4 privacy review): "sync-rules" corrected to Sync Streams and the warden-scoping note corrected to describe the actual, wider PowerSync scope (every assignment ever made, no date filter) versus the API's date-pinned one; the visitor retention note corrected to `visitor_expires_at` **plus** `visitor_retention_days`, not `visitor_expires_at` alone, and to note the registration audit row's own narrowed payload (Document 26); `offline_login_grace_hours` marked as a reserved, not-yet-consulted key; the Generated Reports row's Report Viewer access (finding 2.7, already closed as correct in Document 27) restored after being reverted by an edit outside this document's own revision history.
...
**Notes:**
- A Safety Warden's dashboard visibility is scoped to their own `WardenAssignment` rows, enforced independently at two layers, which do not scope identically. At the **API layer**, every accountability query filters by the requesting user's assignments *as of the activation's `started_at`* — fixed for the whole activation (docs/DECISIONS.md, "a warden's scope is fixed at activation start"). At the **PowerSync Sync Streams layer** (not "sync rules" — PowerSync's current mechanism, confirmed Prompt 10), a warden's device replicates rows for **every zone/area assignment that user has ever had, past, current or future** — PowerSync's stream query engine cannot evaluate `now()`/date comparisons in anything that determines row membership, so the "currently effective" date filter the API applies has no equivalent at this layer (docs/DECISIONS.md, "the warden's 'currently effective' filter cannot be evaluated by the sync engine"). Both layers still prevent a compromised or modified client from asking for a *different* warden's zones, which is the actual security property this note is about; the difference is in how tightly each layer bounds *time*, not identity.
...
- The mobile app is intended to cache the current valid token to permit continued use during a network outage (FR-USR-04), for a configurable grace period (`Setting: offline_login_grace_hours`); once that grace period expires without successful re-verification, the app would require re-authentication before further accountability-affecting actions. As of Stage B's completion, `offline_login_grace_hours` is a **reserved key** — documented as part of the design, not yet consulted by any code path, since the mobile app itself (Stage C) hasn't been built (Document 25, finding 7.4). This paragraph describes the intended behaviour once it is.
...
- **Visitor retention.** A scheduled Oban job runs daily, finds `Person` records of type `visitor` eligible for purge — `visitor_expires_at` **plus** the configured `visitor_retention_days` (default 90) has passed, not `visitor_expires_at` alone; the retention period is how much longer a visitor's personal data is kept *after* their pass expired, not how long the pass itself is valid for — and deletes their personal fields (name, host, contact), retaining only an anonymised placeholder so historical `AccountabilityEvent` rows referencing them remain intact for compliance history without exposing who the visitor was. This job's own execution is audited (NFR-PRIV-01). The `visitor.registered` audit row written at registration time is scoped to the same discipline from the start, not just at purge: it carries only `person_id`, the pass code, and `visitor_expires_at` — never name, phone, email or host — so there is no separate, unpurged copy of a visitor's personal data sitting in `audit_logs` indefinitely (Document 26; docs/DECISIONS.md, "the visitor-registration audit row kept personal data the purge job never reached").
