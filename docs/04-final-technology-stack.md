...
**Status:** Confirmed. This document supersedes the stack table in section 1 of the Technical Foundation document (03). Everything else in that document (architecture pattern, domain model, feature scope, development sequence) still applies, with the adjustments noted in section 4 below. Revised 14 September 2026 (Document 27): "sync rules" corrected to Sync Streams in two places.
...
- **Section 7, Stage B, item 12** ("Sync module: snapshot endpoint, batched event ingest, per-device cursors") is replaced by: configure PowerSync Sync Streams against the Ecto schema, and expose the write endpoints PowerSync's client calls through. (Built as Prompt 10, per Document 03 §7.1's item-to-prompt mapping, added Document 27.)
